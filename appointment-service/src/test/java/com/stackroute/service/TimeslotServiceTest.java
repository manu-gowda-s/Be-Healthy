...
import com.stackroute.appointmentService.service.TimeslotServiceImpl;
...
public class TimeslotServiceTest {
    ...
    public void createSlotTest(){
        Timeslot time = new Timeslot(3,"shashank@gmail.com",LocalDate.now(),LocalTime.now(),LocalTime.now().plusHours(2).plusMinutes(0),true);

        when(timeslotRepository.save(time)).thenReturn(time);

        assertEquals("shashank@gmail.com",timeslotService.createSlot(time).getDoctorEmail());
    }

    @Test
    public void updateSlotAvailabilityTest(){

        Timeslot slot = slot2;
        slot.setSlotAvailability(false);
     when(timeslotRepository.findById(2)).thenReturn(Optional.ofNullable(slot));
     when(timeslotRepository.save(slot)).thenReturn(slot);

     assertEquals(slot2.getSlotAvailability(),timeslotService.updateSlotAvailabilityBySlotId(2).getSlotAvailability());
    }
    ...
    public void getSlotsTest() {
        ...
        assertEquals(doctorEmail,timeslotService.getSlotByDateAndDoctorEmail(slotDate,doctorEmail).get(0).getDoctorEmail());
